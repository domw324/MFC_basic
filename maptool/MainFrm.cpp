...
// 	ON_COMMAND(ID_FILE_SAVE_AS, &CFileManager::OnFileSave)
END_MESSAGE_MAP()
...
// 		pContext);
	...
	if (!m_wndSplitter.CreateView(0, 0, RUNTIME_CLASS(CmaptoolView), CSize(768, 768), pContext))
	{
		return FALSE;
	}

	if (!m_wndSplitter.CreateView(0, 1, RUNTIME_CLASS(CControlView), CSize(1280 - 768, 768), pContext))
	{
		return FALSE;
	}
